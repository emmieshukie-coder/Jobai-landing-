...
function renderJobs(jobs){
  if(!jobs.length){
    document.getElementById("jobs").innerHTML = "<div>No jobs found.</div>";
    return;
  }
    ...
    return \`
      <div class="job-card">
        ...
        <p class="job-meta">
          ...
          <span>\${j.company}</span> • 
          <span>\${timeAgo(j.date_posted)}</span>
        </p>
      </div>
    \`;
  }).join("");
...
function renderUserAds(ads){
  if(!ads.length){
    document.getElementById("userAds").innerHTML = "<div>No community posts yet.</div>";
    return;
  }
  document.getElementById("userAds").innerHTML = ads.map(j => {
    const timeHtml = timeAgo(j.created_at) ? \`<span class="source-tag">\${timeAgo(j.created_at)}</span>\` : "";
    return \`
      <div class="job-card">
        <span class="country-tag user-ad-tag">Community</span>
        \${timeHtml}
        <h3>\${j.title}</h3>
        <p class="job-meta">
          <span>\${j.location}</span> • 
...
async function loadJobs(){
  try{
    ...
    document.getElementById("jobs").innerHTML = "Failed to load jobs";
  }
}
...
// Mock jobs for testing. Replace with your real /jobs route later
